...
/*
  ...
  @tailrec
  private def timesChar(x: Char, xs: List[Char], acc: Int): (Char, Int) = xs match
    case y :: ys => if (x == y) timesChar(x, ys, acc + 1) else timesChar(x, ys, acc)
    case Nil => (x, acc)
 */
  def times(chars: List[Char]): List[(Char, Int)] = {
    val charGroups = chars.groupBy(identity) // Group characters by identity
    val charCounts = charGroups.map { case (char, group) => (char, group.length) }
    charCounts.toList
  }
...
*/
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    // Map each (char, frequency) pair to a Leaf node
    val leafNodes = freqs.map { case (char, freq) => Leaf(char, freq) }

    // Sort the Leaf nodes by ascending weights (frequencies)
    val sortedLeafNodes = leafNodes.sortBy(_.weight)
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case t1 :: t2 :: rest => makeCodeTree(t1, t2) :: rest
    case _ => trees
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    ...
    def decodeHelper(currentTree: CodeTree, remainingBits: List[Bit], acc: List[Char]): List[Char] = {
      currentTree match {
        case Leaf(char, weight) => decodeHelper(tree, remainingBits, acc :+ char)
   ...
   */
  def codeBits(table: CodeTable)(char: Char): List[Bit] = table.flatMap((char, list) => list)
  ...
  def convert(tree: CodeTree): CodeTable ={
    def convertHelper(tree: CodeTree, bits: List[Bit]): CodeTable = tree match {
      ...
      case Fork(left, right, chars, weight) =>
        convertHelper(left, 0 :: bits) ::: convertHelper(right, 1 :: bits)
    }
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] =
    ...
    def quickEncodeHelper(acc: List[Bit], char: Char): List[Bit] = {
      val bits = codeBits(codeTable)(char)
      acc ::: bits
    }
    text.foldLeft(List[Bit]())((acc, char) => quickEncodeHelper(acc, char))
